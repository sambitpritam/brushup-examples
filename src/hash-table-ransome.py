...
def ransome(magazine:str, note:str):
    magazine_dict = dict()
    notes_dict = dict()
    result = "No"
    ...
    for i in note:
        notes_dict[i] = notes_dict.get(i,0) + 1
    
    for k in notes_dict:
        if notes_dict.get(k, 0) <= magazine_dict.get(k, 0):
            result = "Yes"
        else:
            result = "No"
